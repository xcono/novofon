...
func (p *Parser) parseParameterRow(cells *goquery.Selection, isRequest bool) *models.Parameter {
	...
	// Determine array item type if this is an array parameter
	if param.Type == "array" {
		param.ArrayItemType = p.determineArrayItemType(param)
	}
...
// determineArrayItemType determines the type of array items based on parameter information
func (p *Parser) determineArrayItemType(param *models.Parameter) string {
	// Check AllowedValues for type hints
	allowedValues := strings.ToLower(param.AllowedValues)
	description := strings.ToLower(param.Description)
	name := strings.ToLower(param.Name)

	// Check AllowedValues patterns
	switch {
	case strings.Contains(allowedValues, "адрес") || strings.Contains(allowedValues, "email"):
		return "string"
	case strings.Contains(allowedValues, "номер") || strings.Contains(allowedValues, "phone"):
		return "string"
	case strings.Contains(allowedValues, "максимум") && strings.Contains(allowedValues, "адрес"):
		return "string"
	case strings.Contains(allowedValues, "максимум") && strings.Contains(allowedValues, "номер"):
		return "string"
	}

	// Check description patterns
	switch {
	case strings.Contains(description, "идентификатор") || strings.Contains(description, "id"):
		return "number"
	case strings.Contains(description, "номер") && strings.Contains(description, "формат e164"):
		return "string"
	case strings.Contains(description, "адрес") && strings.Contains(description, "почт"):
		return "string"
	case strings.Contains(description, "групп") && strings.Contains(description, "идентификатор"):
		return "number"
	}

	// Check parameter name patterns
	switch {
	case strings.Contains(name, "phone") || strings.Contains(name, "number"):
		return "string"
	case strings.Contains(name, "email"):
		return "string"
	case strings.Contains(name, "group") || strings.Contains(name, "member"):
		return "number"
